...
int scanRegion(int* index, int ind, int luku, HANDLE* phandle, DWORD start, DWORD end) {
	int indeksi = *index;
	int value;
	for (DWORD i = start; i < end; i+=4) // scan through memory from start till end 
	{
...
int main(int argc, char* argv[])
{
	...
	std::thread t1(scanRegion, &indeksi1[0],0, luku, &phandle, address,  0x200000);			// Start 8 threads to go through process image
